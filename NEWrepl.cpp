...
int main() {
    ...
    //continues accepting lines of input until the user enters quit or logout
    while ((input.compare("quit")) != 0 && (input.compare("logout")) != 0) {
        ...
        for (int i = 0; i < input.length();) {
            ...
            if (inputChars[i] != '|' && inputChars[i] != '"') {
                ...
                int temp = i;
                i += (sizeWord - temp);
                while (sameCommand && inputChars[i] != '\0') {
                    while (inputChars[i] == ' ')
                        i++;
                    if (inputChars[i] != '|' && inputChars[i] != '"') {
                        ...
                        while (inQuote) {
                            if (inputChars[i] == '\0') {
                                cout << "Error - expression after \" never completed. Please try again." << endl;
                                printError = true;
                                inQuote = false;
                            }
                            if (inputChars[i] != '"') {
                        ...
                        myProcess newProcess(commandName, listArguments, (listArguments.size() + 1));
                ...
                cout << "Error: invalid input - cannot lead off with '|' or '\"'. Please try again." << endl;
                printError = true;
                break;
            }
        }
        //creating and adding to vector any process not followed by a |
        myProcess newProcess(commandName, listArguments, (listArguments.size() + 1));
        allProcesses.push_back(newProcess);
